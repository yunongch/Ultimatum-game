...
def play():
    ...
    for index in range(len(agentList)):
        ...
        # exit if the agent is the last one
        if index == len(agentList) - 1:
            agentList[index].bank -= k
    ...
    """check if either all agents or (amount - 1) agents are bankrupted"""
    if len(index_bankrupted) == len(agentList) or len(index_bankrupted) == len(agentList) - 1:
