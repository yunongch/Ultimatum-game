...
def inputParams():
    ...
    while True:
        input_temp = input("extra bridges:")
        try:
            input_temp = int(input_temp)
            if input_temp < 0:
                print("extra bridges set to 0")
                extraBridges = 0
                break
            elif input_temp >= agentAmount_left or input_temp >= agentAmount_right:
                print("impossible amount for extra bridges!")
            else:
                extraBridges = input_temp
                break
        except:
...
def getVertices():
    ...
    if extraBridges != 0:
        ...
        for i in range(0, extraBridges):
            ...
            # print("extra:", temp_left_index_list[target_left_index], ":", temp_right_index_list[target_right_index])
            vertices.append((temp_left_index_list[target_left_index], temp_right_index_list[target_right_index]))
            extraBridges_list.append((temp_left_index_list[target_left_index], temp_right_index_list[target_right_index]))
            temp_left_index_list.pop(target_left_index)
            temp_right_index_list.pop(target_right_index)
...
def mimic(index):
    ...
    # TODO
    if agent_mimic.bank < 0:
        print(index, "has a invalid bank")
...
while repeat <= 30:
    ...
    initAgents()
    getVertices()
    print("Original Data for repeat", repeat)
    file_log.flush()

    while sbStillGotMoney and count_round <= 300:
